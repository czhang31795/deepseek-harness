...
- 折叠时，只要还注册了非引导 tab 类型，Sidebar 只是头部角落的一个按钮：会话区保持全宽、滚动条停在自己的边缘，面板展开或只剩引导页时按钮离场但占位保留，头部其余内容不动。[能力占用](../architecture/2026-09-16-capability-occupancy-hides-coding-chrome.zh.md) 拥有隐藏规则。
