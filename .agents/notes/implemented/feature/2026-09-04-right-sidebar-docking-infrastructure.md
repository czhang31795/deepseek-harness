...
- Collapsed, the Sidebar is one header-corner button while a non-guide tab type is registered: the conversation keeps its full width and its scrollbar at its edge, the button leaves when the panel opens or when only the guide remains, and its footprint stays so nothing else in the header moves. [Capability occupancy](../architecture/2026-09-16-capability-occupancy-hides-coding-chrome.md) owns the hide rule.
